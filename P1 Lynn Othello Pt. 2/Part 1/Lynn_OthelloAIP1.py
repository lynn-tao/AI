...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  8 09:41:23 2021
...
# Early in the game, does a higher score mean more mobility?
# Does possessing a corner give a big bump to the score?
# Does possessing a corner-adjacent space hurt the score? What if you possess that corner, is there still a penalty then? How should that work?
# At the end of the game, is a guaranteed victory unquestionably more points than any intermediate state?
# At the end of the game, is a bigger wipeout valued more highly than a narrow win?
...
def minimax(board, player, depth):
    ...
    for position in avail_pos:
        if player == "x":
            next_board = make_move(board, "x", position)
            results = min_step(next_board, depth-1)
            max_posresults.append([position,results])
        elif player == "o":
            next_board = make_move(board, "o", position)
            results = max_step(next_board, depth-1)
            min_posresults.append([position, results])
    if player == "x":
        return find_max_pos(max_posresults)
    else:
        return find_min_pos(min_posresults)
...
def max_step(board, depth): 
    ...
    # no available moves
        ...
        results.append(min_step(board, depth-1))
...
def min_step(board, depth): 
    ...
    # no available moves
        ...
        results.append(max_step(board, depth-1))
...
def find_next_move(board, player, depth):
     move = minimax(board, player, depth)
     return move
